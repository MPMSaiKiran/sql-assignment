...
SELECT
  ...
  -- Previous payment in same type & installment group
  LAG(payment_value) OVER (
    PARTITION BY payment_type, payment_installments
    ORDER BY payment_value
  ) AS prev_payment,

  -- Next payment in same type & installment group
  LEAD(payment_value) OVER (
    PARTITION BY payment_type, payment_installments
    ORDER BY payment_value
  ) AS next_payment,

  -- First payment value in this group
  FIRST_VALUE(payment_value) OVER (
    PARTITION BY payment_type, payment_installments
    ORDER BY payment_value
  ) AS first_payment,

  -- Last payment value in group (requires full window frame)
  LAST_VALUE(payment_value) OVER (
    PARTITION BY payment_type, payment_installments
    ORDER BY payment_value
    ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
  ...
  -- Rank based on value within payment type & installments
  RANK() OVER (
    PARTITION BY payment_type, payment_installments
    ORDER BY payment_value DESC
  ) AS rank_in_group,

  -- Running total for payment_value
  SUM(payment_value) OVER (
    PARTITION BY payment_type, payment_installments
    ORDER BY payment_value
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
...
FROM olist_order_payments
ORDER BY payment_type, payment_installments, payment_value;
